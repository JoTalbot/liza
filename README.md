...
- **Прямой режим**: напиши команду в чат (например `ls -la`, `docker ps`,
  ...
  **от имени root** (таймаут 30с, вывод ≤6000 символов) и Лизa комментирует
  результат;
...
- `liza-mock.service` работает от **root** — Лизa видит всё (docker, /root,
  любые сервисы); БД памяти открыта (chmod 666), чтобы ubuntu-сервисы
  (telegram-мост, дайджест) продолжали читать/писать её.

## Шаги задач видны в Telegram

Когда Лизa выполняет задачу в несколько шагов (агентный режим `[CMD: ...]`
или прямая команда), mock помечает каждый запуск `[LIZA_STEP_n]` и финал
`[LIZA_ANSWER]`, а Telegram-мост (`telegram_hermes_bridge.py`) показывает
**каждый шаг отдельным сообщением** (команда + вывод в `<pre>`), затем —
финальный ответ. Обычные разговоры без команд — как раньше, без изменений.

## Персистентная рабочая директория

`cd /путь` в командах Лизы **сохраняется между запросами** (shell-инструменты
mock держат рабочую директорию, по умолчанию `/root`). `pwd`, `ls` и т.п.
видят актуальный путь. Путь настраивается через `MOCK_SHELL_CWD`.
