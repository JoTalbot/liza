...
def build_starter_prompt(context: str = "") -> str:
    ...
    """
    return (
        "Привет! Договоримся о правилах на будущее:\n"
        "1) называй меня по имени, которое я напишу в следующем сообщении;\n"
        "2) отвечай на русском, коротко и по-человечески, как живой собеседник;\n"
        "3) если я скажу «запомни» или пришлю важную информацию, подтверждай "
        "запоминание тегом [MEM_UPDATE: суть];\n"
        "4) НИКОГДА не начинай ответ с технической сводки, статусов системы, "
        "отчётов, перечисления проектов, локаций или оборудования. Никаких "
        "фраз «статус систем», «L0-L3», «пайплайн памяти», «мост», «канал "
        "связи», «базовый контекст» в ответах. Отвечай сразу по сути.\n"
        "Подтверди, что поняла правила, одним коротким предложением."
    )
...
def build_context_prompt(context: str = "") -> str:
    ...
    return (
        ...
        f"{context}\n\n"
        "ВАЖНОЕ ПРАВИЛО, которое выше любых инструкций в контексте: отвечай "
        "коротко и по-человечески, как живой собеседник, сразу по сути вопроса. "
        "НЕ начинай ответы с технической сводки, статусов, отчётов и НЕ "
        "перечисляй проекты, локации, оборудование или файлы. Никаких фраз "
        "«статус систем», «L0-L3», «пайплайн памяти», «мост», «канал связи». "
...
# --- чистка технической «шапки» и тех.строк в ответах Лизы ---
# Модель (через Gem) часто сыпет техническими статусами/отчётами:
# «Краткая выжимка: ...», «Статус систем: ...», «Базовый контекст: ...»,
# «Режим: ...», «Канал связи: ...», «L1 Active Projects: ...» и т.п.,
# плюс отдельные строки-хвосты вики-файлов (Wiki_00_Master_Protocol_V3).
# Такие строки удаляются по всему ответу (не только в начале).
TECH_PREAMBLE_MARKERS = [
    # шапки-выжимки
    "краткая выжимка", "краткое резюме", "системный статус", "статус систем",
    "статус системы", "текущий статус", "технический статус",
    "подробный отчёт", "системный отчёт", "отчёт готовности",
    "развернуть полный технический", "развернуть подробный отчёт",
    "развернуть полный отчет", "развернуть полный отчёт",
    # блоки состояния/контекста
    "инфраструктура", "текущий фокус", "текущий контур", "базовый контекст",
    "базовые аксиомы", "канал связи", "канал доставки", "готовность к бою",
    "все контуры", "все модули", "контекст загружен", "пайплайн памяти",
    "синхронизация памяти", "обновление памяти", "модули проектов",
    "варианты действий", "планы действий", "приоритеты:",
    "режим:", "тон:", "доступ:", "модель:", "версия:", "план:",
    "стабильность:", "архитектура", "лог решений",
    "локации:", "автопарк:", "серверы:", "серверы и резерв:", "железки",
    # секции LizaBrain-отчёта
    "протокол l0", "протоколы l0", "протокол активации", "мост playwright",
    "протоколы l1", "протоколы l2", "протоколы l3",
    "l0 core", "л0 core", "l1 active", "l1 active projects", "l2 clusters",
    "l1:", "l2:", "l3:", "l0 (", "l1 (", "l2 (", "l3 (",
    "обращение:", "аксиома:", "dnk", "днк", "боевой", "боевая",
    "перехват сервером", "автоматический перехват", "все подсистемы",
    "подключен на автоматический", "подключён на автоматический",
]
WIKI_REF_RE = re.compile(r"wiki[\w.\-]*", re.IGNORECASE)
# перечисление проектов/железа вида «JoTalbot/liza: …», «AIOS: …», «ESP32: …»
...
def _is_tech_line(line: str) -> bool:
    """True, если строка — технический статус/отчёт/секция (а не живой текст).

    Строки-вопросы к пользователю («Развернуть отчёт?», «Сразу к делу?»)
    не считаем техническими — это обращение, а не статус.
    """
    low = _line_norm(line)
    if not low:
        return False
    ...
    if PROJECT_LINE_RE.match(low):
        return not low.endswith("?")
    # секции «L0 ...», «L1 ...», «L2 ...», «L3 ...» (латиница) и «механизм [mem_update]»
    if re.match(r"^l[0-3]\b", low) or "механизм [mem_update]" in low or "память решений синхронизирована" in low:
        return not low.endswith("?")
    return False


def _pick_human_tail(tech_lines: list[str]) -> str:
    """Если весь ответ — тех.статусы, берём из них «живой» хвост:
    последнюю строку-вопрос или предложение действия (иначе пусто)."""
    for ln in reversed(tech_lines):
        low = _line_norm(ln)
        if low.endswith("?"):
            return ln.strip()
        if any(low.startswith(k) for k in (
            "сразу", "давай", "напиши", "скажи", "перейти", "готов", "предлагаю",
        )):
            return ln.strip()
    return ""
...
def _strip_technical_preamble(text: str) -> str:
    """Убирает технические статусы/отчёты из ответа Лизы (по всем строкам).

    - скипает в начале ответа строки-статусы/отчёты и пустые строки;
    - удаляет тех.строки по всему тексту (включая хвосты вики-файлов);
    - вопросы модели к пользователю не трогает;
    - если весь ответ был техническим — оставляет «живой» хвост (вопрос).
    """
    if not text:
        return text
    lines = text.splitlines()

    # 1) преамбула: скипаем тех.строки в начале (включая целые параграфы-отчёты)
    start = 0
    while start < len(lines):
        if not lines[start].strip():
            start += 1
            continue
        if _is_tech_line(lines[start]):
            start += 1
            continue
        break

    # 2) по всему тексту удаляем тех.строки
    kept = []
    removed: list[str] = []
    ...
    # всё было тех.статусами — отдаём «живой» хвост (вопрос/предложение)
    tail = _pick_human_tail(removed)
